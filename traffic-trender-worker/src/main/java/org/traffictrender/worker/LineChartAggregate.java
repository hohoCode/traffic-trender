...
public class LineChartAggregate {
	...
	if (zoom.getState() == null && zoom.getCounty() == null && zoom.getRoad() == null ) { ///All State
	    sqlString = "FROM " + db.getClause();
	    sqlString = prepareFilterClause(filter, sqlString);	    
	    whichLevel = "state";
	} else if (zoom.getState() != null && zoom.getCounty() == null && zoom.getRoad() == null ) { ////All County
	    sqlString = "FROM " + db.getClause();
	    sqlString = prepareFilterClause(filter, sqlString);	    
	    ...
	    whichLevel = "county";
	} else if (zoom.getState() != null && zoom.getCounty() != null && zoom.getRoad() == null ) { ///All Road
	    sqlString = "FROM " + db.getClause();
	    sqlString = prepareFilterClause(filter, sqlString);
	...
	} else if (zoom.getState() != null && zoom.getCounty() != null && zoom.getRoad() != null ) { //All Locations
	    sqlString = "FROM " + db.getClause();
	    sqlString = prepareFilterClause(filter, sqlString);	    
    ...
    private static Map<String,Map<Integer, Map<Integer, Object>>> yearMonthIteration(String sql, MeasurementType threeGuy, MysqlConnect db, String whichLevel) {
	Map<String,Map<Integer, Map<Integer, Object>>> outputMap = new HashMap<String,Map<Integer, Map<Integer, Object>>>();

	try {
	    String query = "SELECT " + whichLevel + " " + sql + " group by " + whichLevel;
		...
		if (!sql.contains("where")) {
		    query = sql + " where " + whichLevel + " = \'" + targetString + "\' group by year, month, "+whichLevel;
		} else {
		    query = sql + " and " + whichLevel +" = \'" + targetString +  "\' group by year, month, "+whichLevel;
		}
		...
		while(locationNumber.next()) {
			...
			//System.out.println("Year:"+yea + " Month:" + monn + " TARGET:" +targetString+" "+ threeGuy +":"+measureDouble);
		    } else {
			int measureInt = locationNumber.getInt("output");
			m1.put(monn, (Object)measureInt);
			//System.out.println("Year:"+yea + " Month:" + monn + " TARGET:" +targetString+" "+ threeGuy +":"+measureInt);
		    }
		    m2.put(yea, m1);
	...
	inputFilter.add(new Location("SC", "GREENVILLE", "I-185", null));
	inputFilter.add(new Location("SC", "GREENVILLE", "I-385", null));
	LineChartAggregate.generatorResults(new Location("SC", "GREENVILLE", "I-185", null), inputFilter, MeasurementType.length);
    }*/
}
